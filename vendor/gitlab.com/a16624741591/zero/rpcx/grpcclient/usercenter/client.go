...
// HandleGetUser 客户端处理逻辑
func (c *GrpcClient) HandleGetUser(ctx context.Context, req GetUserRequest) *GetUserResponse {
	if req.Context == nil {
		panic("grpc context is nil and requestID must be set")
	}
	ctx1, span, requestID := grpcbase.NewTraceSpanFromRequestID("client", "/pb.usercenter/GetUser", req.Context.RequestID)
	defer span.End()
	...
	resp := c.client.GetUser(ctx1, req)
	if resp.Message != "" {
		log.Println("ERROR：请求 GetUser 失败，error：" + resp.Message)
		return nil
	}
	if resp.Data != nil {
		var m = resp.Data.(GetUserResponse)
		return &m
	}
	return nil
}
