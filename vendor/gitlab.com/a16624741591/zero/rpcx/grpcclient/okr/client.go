...
func NewClient() GrpcClient {
	c, err := grpcbase.DialClient(grpcbase.ServerAddr(grpcbase.OkrSVC))
	if err != nil {
		log.Fatal(err)
	}
	client := c.(Repository)
...
// HandleGetOkr 客户端处理逻辑
func (c *GrpcClient) HandleGetOkr(ctx context.Context, req GetOkrRequest) *GetOkrResponse {
	if req.Context == nil {
		panic("grpc context is nil and requestID must be set")
	}
	ctx1, span, requestID := grpcbase.NewTraceSpanFromRequestID("client", "/pb.okr/GetOkr", req.Context.RequestID)
	defer span.End()
	...
	resp := c.client.GetOkr(ctx1, req)
	if resp.Message != "" {
		log.Println("ERROR：请求 GetOkr 失败，error：" + resp.Message)
		return nil
	}
	if resp.Data != nil {
		var m = resp.Data.(GetOkrResponse)
		return &m
	}
	return nil
}
